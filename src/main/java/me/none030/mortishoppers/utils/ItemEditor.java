...
public class ItemEditor {
    ...
    public void setPlaceholder(String placeholder, String value) {
        ...
        Component name = meta.displayName();
        if (name != null) {
            meta.displayName(color(LegacyComponentSerializer.legacyAmpersand().serialize(name).replace(placeholder, value)));
        }
        List<Component> lore = meta.lore();
        if (lore != null) {
            lore.replaceAll(component1 -> color(LegacyComponentSerializer.legacyAmpersand().serialize(component1).replace(placeholder, value)));
            meta.lore(lore);
        }
        item.setItemMeta(meta);
    }
